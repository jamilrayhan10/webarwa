import Link from "next/link";



export default function TeamArea() {
  return (
    <section className="rk-theme-section-padding6">
      <div className="container">
        <div className="rk-theme-section-title center max-width-750" data-aos="fade-up" data-aos-delay="500">
          <h2>Our team consists of a group of talents</h2>
        </div>
        <div className="row">
          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="rk-theme-team-wrap wrap2" data-aos="fade-up" data-aos-delay="500">
              <Link href="/single-team">
                <div className="rk-theme-team-thumb">
                  <img src="assets/images/team/team1.png" alt="Thumb" />
                </div>
                <div className="rk-theme-team-content">
                  <h3>Mr. Abraham Maslo</h3>
                </div>
              </Link>

              <div className="rk-theme-team-author-wrap">
                <div className="rk-theme-team-author-data">
                  <p>Chief AI Officer</p>
                </div>
                <div className="rk-theme-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="rk-theme-team-wrap wrap2" data-aos="fade-up" data-aos-delay="700">
              <Link href="/single-team">
                <div className="rk-theme-team-thumb">
                  <img src="assets/images/team/team2.png" alt="Thumb" />
                </div>
                <div className="rk-theme-team-content">
                  <h3>Willium Robert</h3>
                </div>
              </Link>

              <div className="rk-theme-team-author-wrap">
                <div className="rk-theme-team-author-data">
                  <p>Data Engineer</p>
                </div>
                <div className="rk-theme-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="rk-theme-team-wrap wrap2" data-aos="fade-up" data-aos-delay="900">
              <Link href="/single-team">
                <div className="rk-theme-team-thumb">
                  <img src="assets/images/team/team3.png" alt="Thumb" />
                </div>
                <div className="rk-theme-team-content">
                  <h3>Henry Fayol</h3>
                </div>
              </Link>
              <div className="rk-theme-team-author-wrap">
                <div className="rk-theme-team-author-data">
                  <p>Research Scientist</p>
                </div>
                <div className="rk-theme-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>

          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="rk-theme-team-wrap wrap2" data-aos="fade-up" data-aos-delay="500">
              <Link href="/single-team">
                <div className="rk-theme-team-thumb">
                  <img src="assets/images/team/team4.png" alt="Thumb" />
                </div>
                <div className="rk-theme-team-content">
                  <h3>Henry Martine</h3>
                </div>
              </Link>
              <div className="rk-theme-team-author-wrap">
                <div className="rk-theme-team-author-data">
                  <p>AI Researchers</p>
                </div>
                <div className="rk-theme-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="rk-theme-team-wrap wrap2" data-aos="fade-up" data-aos-delay="700">
              <Link href="/single-team">
                <div className="rk-theme-team-thumb">
                  <img src="assets/images/team/team5.png" alt="Thumb" />
                </div>
                <div className="rk-theme-team-content">
                  <h3>Jack Fox</h3>
                </div>
              </Link>
              <div className="rk-theme-team-author-wrap">
                <div className="rk-theme-team-author-data">
                  <p>NLP Expert</p>
                </div>
                <div className="rk-theme-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="rk-theme-team-wrap wrap2" data-aos="fade-up" data-aos-delay="900">
              <Link href="/single-team">
                <div className="rk-theme-team-thumb">
                  <img src="assets/images/team/team6.png" alt="Thumb" />
                </div>
                <div className="rk-theme-team-content">
                  <h3>Adam Smith</h3>
                </div>
              </Link>
              <div className="rk-theme-team-author-wrap">
                <div className="rk-theme-team-author-data">
                  <p>Project Manager</p>
                </div>
                <div className="rk-theme-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>

          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="rk-theme-team-wrap wrap2" data-aos="fade-up" data-aos-delay="500">
              <Link href="/single-team">
                <div className="rk-theme-team-thumb">
                  <img src="assets/images/team/team7.png" alt="Thumb" />
                </div>
                <div className="rk-theme-team-content">
                  <h3>Joo Bitler</h3>
                </div>
              </Link>
              <div className="rk-theme-team-author-wrap">
                <div className="rk-theme-team-author-data">
                  <p>Cyber Expert</p>
                </div>
                <div className="rk-theme-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="rk-theme-team-wrap wrap2" data-aos="fade-up" data-aos-delay="700">
              <Link href="/single-team">
                <div className="rk-theme-team-thumb">
                  <img src="assets/images/team/team8.png" alt="Thumb" />
                </div>
                <div className="rk-theme-team-content">
                  <h3>Homi Corn</h3>
                </div>
              </Link>
              <div className="rk-theme-team-author-wrap">
                <div className="rk-theme-team-author-data">
                  <p>HR Manager</p>
                </div>
                <div className="rk-theme-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="rk-theme-team-single" data-aos="fade-up" data-aos-delay="900">
              <h3>You want to join our amazing team</h3>
              <p>Specify the job you are applying for and introduce yourself</p>
              <div className="mt-50 w-100">
                <Link className="rk-theme-default-btn d-block" href="/single-team" data-text="Join our team">
                  <span className="button-wraper">Join our team</span>
                </Link>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  )
}
